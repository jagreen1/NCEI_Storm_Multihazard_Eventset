...
dfsingle['GEOID'] = dfsingle['STATE_FIPS'].astype(str).str.zfill(2) + dfsingle['CZ_FIPS'].astype(str).str.zfill(3)
...
dfsingle = dfsingle.reindex(columns=['EPISODE_ID', 'EVENT_ID', 'GEOID', 'STATE', 'STATE_FIPS',
       'EVENT_TYPE', 'HAZARD', 'CZ_TYPE', 'CZ_FIPS', 'CZ_NAME', 'BEGIN_DATETIME', 'END_DATETIME', 'start_year', 'end_year', 'WFO', 'CZ_TIMEZONE',
       ...
       'DATA_SOURCE', 'EPISODE_NARRATIVE', 'EVENT_NARRATIVE'])

#temporal filter
dfsingle = dfsingle[(dfsingle['BEGIN_DATETIME'].dt.year >= start_year) & (dfsingle['END_DATETIME'].dt.year <= end_year)]
...
Exclusion_State_List = ['ATLANTIC SOUTH', 'LAKE ST CLAIR', 'ALASKA', 'GUAM', 'LAKE MICHIGAN', 'LAKE ONTARIO', 'PUERTO RICO', 'GULF OF MEXICO', 'ATLANTIC NORTH',  'VIRGIN ISLANDS', 'LAKE HURON', 'AMERICAN SAMOA', 'ST LAWRENCE R', 'LAKE ERIE', 'LAKE SUPERIOR', 'E PACIFIC']
...
dfsingle = dfsingle[((dfsingle['INJURIES_DIRECT'] >= inj) | (dfsingle['INJURIES_INDIRECT'] >= inj) | (dfsingle['DEATHS_DIRECT'] >= dth) | (dfsingle['DEATHS_INDIRECT'] >= dth) | (dfsingle['DAMAGE_CROPS'] >= c * 1000) |(dfsingle['DAMAGE_PROPERTY'] >= p * 1000))]

#save prepared singledf, as csv and parquet
dfsingle.to_csv(rf'{Output_Eventset_Path}\NCEI-Storm_singlehazards\dfsingle_{inj}inj_{dth}dth_{c}c_{p}p_filtered_timelag{time_lag_int}_{start_year}-{end_year}.csv.gz',compression='gzip', encoding='utf-8',index=False)
dfsingle.to_parquet(rf'{Output_Eventset_Path}\NCEI-Storm_singlehazards\dfsingle_{inj}inj_{dth}dth_{c}c_{p}p_filtered_timelag{time_lag_int}_{start_year}-{end_year}.parquet.gzip',compression='gzip')
...
# Check if all values in the group are the same
    if (x == x.iloc[0]).all():
        ...
        return ','.join(map(str, x)) #If different, combine as a string with a delimiter
...
for state_fips in tqdm(state_fips_list):
    ...
    for county_fips in tqdm(county_fips_list):
        ...
        for idx, row in df.iterrows():
            ...
            subset = df[(df['CZ_FIPS'] == row['CZ_FIPS']) &
            ...
            for _, other_row in subset.iterrows():
                if row['EVENT_ID'] != other_row['EVENT_ID'] and datetime_ranges_overlap_with_lag(row['BEGIN_DATETIME'],row['END_DATETIME'],other_row['BEGIN_DATETIME'],other_row['END_DATETIME'],time_lag):
                    df.at[idx, 'OVERLAPPING_EVENTS'].append((other_row['EVENT_ID']))
                    if row['EVENT_TYPE'] != other_row['EVENT_TYPE']:
                        overlapping_event_pairs.append(((row['EVENT_ID']),(other_row['EVENT_ID'])))
        
        # Convert list to string for easier reading
        #df['OVERLAPPING_EVENTS'] = df['OVERLAPPING_EVENTS'].apply(lambda x: ', '.join(map(str, x)))
        df.loc[:,'OVERLAPPING_EVENTS'] = df['OVERLAPPING_EVENTS'].apply(lambda x: ','.join(map(str, x)))
        
        df = df[['OVERLAPPING_EVENTS'] + [col for col in df.columns if col != 'OVERLAPPING_EVENTS']]
        ...
        if len(overlapping_event_pairs_unique)>0: 
            ...
            for i in range(0,len(overlapping_event_pairs_unique)):
                pair_df_1 = df[df['EVENT_ID'] == overlapping_event_pairs_unique[i][0]][0:1]
                ...
                temp_df = temp_df.sort_values(by='EVENT_TYPE') #reorder the two event pairs such that the event_type pairs are later formatted the same, when combined into a single string
                all_pair_df = pd.concat([all_pair_df,temp_df])
                            
            
            all_pair_df = all_pair_df[['PAIR_ID'] + [col for col in all_pair_df.columns if col != 'PAIR_ID']]
            all_pair_df = all_pair_df.drop_duplicates()
        
    
            MULTI_INJURIES_DIRECT = all_pair_df.groupby('PAIR_ID')['INJURIES_DIRECT'].transform('sum')
            MULTI_INJURIES_INDIRECT = all_pair_df.groupby('PAIR_ID')['INJURIES_INDIRECT'].transform('sum')
            MULTI_DEATHS_DIRECT = all_pair_df.groupby('PAIR_ID')['DEATHS_DIRECT'].transform('sum')
            MULTI_DEATHS_INDIRECT = all_pair_df.groupby('PAIR_ID')['DEATHS_INDIRECT'].transform('sum')
            MULTI_DAMAGE_PROPERTY = all_pair_df.groupby('PAIR_ID')['DAMAGE_PROPERTY'].transform('sum')
            MULTI_DAMAGE_CROPS = all_pair_df.groupby('PAIR_ID')['DAMAGE_CROPS'].transform('sum')
            ...
            all_pair_df = all_pair_df.reindex(columns=['PAIR_ID','OVERLAPPING_EVENTS','EPISODE_ID','EVENT_ID', 'GEOID', 'STATE','STATE_FIPS',
                                 'EVENT_TYPE', 'HAZARD', 'CZ_TYPE', 'CZ_FIPS', 'CZ_NAME', 'BEGIN_DATETIME', 'END_DATETIME', 'start_year', 'end_year',
                                 'WFO', 'CZ_TIMEZONE',
...
dfmulti.to_csv(rf'{Output_Eventset_Path}\NCEI-Storm_multihazards\dfmulti_{inj}inj_{dth}dth_{c}c_{p}p_filtered_timelag{time_lag_int}_{start_year}-{end_year}.csv.gz',compression='gzip', encoding='utf-8',index=False)
dfmulti.to_parquet(rf'{Output_Eventset_Path}\NCEI-Storm_multihazards\dfmulti_{inj}inj_{dth}dth_{c}c_{p}p_filtered_timelag{time_lag_int}_{start_year}-{end_year}.parquet.gzip',compression='gzip')
